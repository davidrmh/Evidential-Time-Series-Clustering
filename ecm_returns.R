...
tsne_adj <- Rtsne(ret_adj, dims = 2, perplexity = perp)
tsne_ohcl <- Rtsne(ret_ohcl, dims = 2, perplexity = perp)
...
# Evidential c-means with Adj Close prices
...
# Evidential c-means with OHCL prices
...
plot(tsne_adj$Y, xlab = "X", ylab ="Y", main = "K-means-Adj Close",
     col = km_adj$cluster, pch = 19)

# Plot ECM using Adj Close Prices and Plausibility
plot(tsne_adj$Y,xlab = "X", ylab = "Y", main = "ECM-Plausibility-Adj Close",
     col = ecm_pl_adj$cluster, pch = 19)

# Plot ECM using Adj Close Prices and Belief
plot(tsne_adj$Y,xlab = "X", ylab = "Y", main = "ECM-Belief-Adj Close",
     col = ecm_bel_adj$cluster, pch = 19)

# Plot K-Means using OHCL prices
plot(tsne_ohcl$Y, xlab = "X", ylab ="Y", main = "K-means-OHCL",
     col = km_ohcl$cluster, pch = 19)

# Plot ECM using OHCL Prices and Plausibility
plot(tsne_ohcl$Y,xlab = "X", ylab = "Y", main = "ECM-Plausibility-OHCL",
     col = ecm_pl_ohcl$cluster, pch = 19)

# Plot ECM using Adj Close Prices and Belief
plot(tsne_ohcl$Y,xlab = "X", ylab = "Y", main = "ECM-Belief-OHCL",
